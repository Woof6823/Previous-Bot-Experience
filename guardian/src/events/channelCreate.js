...
module.exports = {
  ...
  async execute(channel) {
    ...
    const resolved = await auditService.resolveActor(guild, AuditLogEvent.ChannelCreate, channel.id);
    ...
    await securityService.triggerIncident(guild, {
      actorId,
      actorMember,
      type: "unauthorized_channel_create",
      details: { target: channel.id, channelName: channel.name },
      threshold: "any non-whitelisted channel creation",
      reason: `Channel "${channel.name}" was created by a non-whitelisted user.`
    });
  }
};
